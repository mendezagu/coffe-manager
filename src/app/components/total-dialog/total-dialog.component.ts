    ...
    this.gestionService.resetTable(this.data.tableId).subscribe({
      next: (response) => {
        ...
        const balanceEntry = {
          ...
          totalAmount: this.total,  // Total de la mesa
        };
  
        this.gestionService.addBalanceEntry(balanceEntry).subscribe({
          next: () => console.log('Balance actualizado'),
          error: (error) => console.error('Error al actualizar balance:', error),
        });
            ...
            <div class="totals">
              ...
              ${
                this.discountPercentage > 0
                  ? `<p><strong>Descuento (${this.discountPercentage}%):</strong> -$${(
                      (this.total * this.discountPercentage) / 100
                    ).toFixed(2)}</p>
